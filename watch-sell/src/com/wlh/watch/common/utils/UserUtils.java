 ...
 */
package com.wlh.watch.common.utils;
...
public class UserUtils {
	...
	public static User get(String id){
		com.wlh.watch.modules.user.entity.User user = (User)CacheUtils.get(USER_CACHE, USER_CACHE_ID_ + id);
		if (user ==  null){
			...
			CacheUtils.put(USER_CACHE, USER_CACHE_LOGIN_NAME_ + user.getEmail(), user);
		}
		return user;
	}
	...
	public static User getByLoginName(String loginName){
		...
		if (user == null){
			User user1 = new User();
			user1.setEmail(loginName);
			user = userDao.getByEmail(user1);
			if (user == null){
				return null;
			}
			//user.setRoleList(roleDao.findList(new Role(user)));
	...
	public static List<SysMenu> getMenuList(){
		...
		if (menuList == null){
			...
			if (user.getId() != null && user.getId().equals("1")){
				...
				menuList = menuDao.getByUserId(user.getId());
			}
